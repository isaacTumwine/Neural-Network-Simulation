...
class KNN {
  ...
  classifyCircleType() {
    if (this.weighted) {
      return this.determineCircleTypeByDistanceWeighted(this.closestNeighbors);
    }
  ...
  determineCircleTypeByDistanceWeighted(neighbors) {
    const result = _.chain(_.groupBy(this.closestNeighbors, n => n.type)).
    mapObject((val, key) => {
      return { inverseDistance: val.reduce((sum, n) => sum + n.inverseDistance, 0), type: key };
    }).
    max(o => o.inverseDistance).
    value();
    return result.type;
  }
  getKBoundingCircle() {
    if (this.circle) {
      ...
      return new Circle(this.circle.cx, this.circle.cy, radius, type = 'None', color = 'transparent');
    }
  }
  getKConnectingLines() {
...
class KNNVisualization {
  constructor(width = 500, height = 300, amountOfCircles = 50, types = ['A', 'B', 'None']) {
    this.width = width;
    this.height = height;
  ...
  onChangeWeightedCheckbox() {
    document.querySelector('#weighted').addEventListener('change', e => {
      this.knn.setWeighted(e.target.checked);
    });
  }
  onClickSVG() {
...
const minHeight = 350;
const w = Math.max(document.documentElement.clientWidth, window.innerWidth || 0) - 40;
const h = minHeight - 30;
const amountOfCircles = 200;
const knnVis = new KNNVisualization(w, h, amountOfCircles);
